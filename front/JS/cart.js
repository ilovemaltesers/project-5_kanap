...
for (let i = 0; i < productsInLocalStorage.length; i++) {
  // Check if the product at index i is not undefined or null
  if (productsInLocalStorage[i]) {
    const product = productsInLocalStorage[i];
    const productId = product.id;
    const productColor = product.color;
    const productQuantity = product.quantity;

    console.log("Product Ident:", productId);
  } else {
    console.error(`Product at index ${i} is undefined or null`);
  }
